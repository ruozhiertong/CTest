...
int recv_arp(int sockfd, struct sockaddr_ll *peer_addr)
{
	...
	char test_ip[5];
	int i = 0;
	memcpy(test_mac,frame.arp.arp_sha, 6);    //源MAC地址
	//test_mac[6] = 0;
	//printf("%s\n", test_mac);
	for(i = 0 ; i < 6; i++)
	{
		printf("%x:",test_mac[i]); //mac地址 十六进制
	}
	printf("\n");
	
    memcpy(test_ip,frame.arp.arp_spa,4);     //源IP地址
    // test_ip[4] = 0;
	// printf("%s\n", test_ip);
	for(i = 0 ; i < 4; i++)
	{
	...
	printf("\n");
    memcpy(test_mac, frame.arp.arp_tha, 6);    //目的MAC地址
    //test_mac[6] = 0;
   	//printf("%s\n", test_mac);
    for(i = 0 ; i < 6; i++)
	{
	...
	printf("\n");
    memcpy(test_ip ,frame.arp.arp_tpa, 4);     //目的IP地址
    // test_ip[4] = 0;
	// printf("%s\n", test_ip);
	for(i = 0 ; i < 4; i++)
	{
